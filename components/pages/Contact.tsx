...
export default function Contact() {
  ...
  const locationCards = [
    {
      title: 'Thane West — Rhymes N Colours',
      subtitle: 'Manpada Branch',
    ...
    {
      title: 'Thane West — Kinder Hive',
      subtitle: 'Vasant Vihar Branch',
    ...
    {
      title: 'Dombivli East — SD Studio',
      subtitle: 'Regency Anantam Branch 1',
      address: 'SD Education hub, Regency Anantam gate no 1, Dombivli (East)',
      mapLink: 'https://maps.app.goo.gl/YEA5qG25toE9D2gPA',
      days: 'Every Monday & Wednesday',
      timing: '5:00 PM – 7:00 PM ',
    },
    {
      title: 'Dombivli East — Kidzdom',
      subtitle: 'Regency Anantam Branch 2',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            {locationCards.map((card, index) => (
              <ScrollReveal key={card.title} variant="slideLeft" delay={0.1 + index * 0.08}>
                <div className="bg-white/20 backdrop-blur-md border-2 border-brand-gold/40 p-6 rounded-2xl">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      <h3 className="font-serif text-xl font-bold text-brand-gold">{card.title}</h3>
                      <p className="font-sans text-brand-gold/70 text-xs uppercase tracking-widest">{card.subtitle}</p>
                    </div>
                    <a
                      href={card.mapLink}
                      target="_blank"
                  ...
                  <div className="space-y-1.5 text-sm">
                    <p className="text-brand-cream"><span className="text-brand-gold font-bold inline-flex items-center gap-1"><CalendarDays className="w-4 h-4" /> Days:</span> {card.days}</p>
                    <p className="text-brand-cream"><span className="text-brand-gold font-bold inline-flex items-center gap-1"><Clock className="w-4 h-4" /> Timing:</span> {card.timing}</p>
          ...
          <ScrollReveal variant="slideRight" delay={0.15}>
            <div className="bg-brand-cream rounded-2xl p-8 shadow-2xl self-start">
