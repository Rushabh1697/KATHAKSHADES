...
function PhotoScrollRow({ category, index }: { category: Category; index: number }) {
  ...
  return (
    <ScrollReveal variant={index % 2 === 0 ? 'slideLeft' : 'slideRight'} delay={index * 0.08} duration={0.6}>
      <div className="mb-20">
        {/* Section header */}
              ...
              {photos.map((src, idx) => (
                ...
                >
                  <div className="relative w-full aspect-[4/3] overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-brand-maroon/60 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <ZoomIn className="w-10 h-10 text-white opacity-80" strokeWidth={1.5} />
                    </div>
                  </div>
                </div>
              ))}
    ...
    <section id="catalogue" className="py-20 bg-brand-cream min-h-screen">
      <div className="container">
        ...
        {categories.map((cat, index) => (
          <PhotoScrollRow key={cat.tag} category={cat} index={index} />
        ))}
